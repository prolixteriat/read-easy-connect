...
class DbUsers extends DbBase {
    ...
    # Optional : affiliate_id, coordinator_id, manager_id, status
    ...
    public function edit_user(Request $request): Status {
        try {
            ...
            if ($result['role'] === 'coach' && $user_status === 'leaver' && 
                $result['status'] !== 'leaver') {
    ...
    public function get_users(Request $request): Status {
        try {
            ...
            # Admins and directors see all users
                ...
                # Managers see users linked by their affiliate (coordinators and coaches) + themselves
                $stmt = $this->conn->prepare("
                    SELECT 
                        $user_fields,
                        COALESCE(
                ...
                # Coordinators see all coaches in their affiliate + all coordinators in their affiliate
    ...
    private function is_allowed_change(int $master_id, int $slave_id): void {
        ...
        # Rule 3: manager can change managers, coordinators, or coaches in same affiliate
        if ($master_role === 'manager') {
            if (!in_array($slave_role, ['manager', 'coordinator', 'coach'])) {
