...
import { ArrowUpDown, Copy, ChevronDown, ChevronRight } from 'lucide-react';
...
export default function EnquiryTable({ data, error }: EnquiryTableProps): React.JSX.Element {
  ...
  const aggregatedData = useMemo(() => {
    ...
    const grouped = data.reduce((acc, referral) => {
      const key = `${referral.org_name}-${referral.area_name || ''}-${referral.status}`;
      if (!acc[key]) {
        acc[key] = {
    ...
    Array.from(orgAreaGroups.entries())
      ...
      .forEach(([groupKey, items]) => {
        ...
        const [org_name, area_name] = groupKey.split('-');
        result.push({
          org_name,
          area_name: area_name || '',
          status: 'TOTAL',
        ...
        // Add detail rows only if expanded
        if (expandedGroups.has(groupKey)) {
          result.push(...items.sort((a, b) => a.status.localeCompare(b.status)));
        }
      });
  ...
  const copyToClipboard = useCallback(async () => {
    ...
    const headers = ['Organisation', 'Area', 'Status', 'Count'];
